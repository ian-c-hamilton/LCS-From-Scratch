...
# and `pip install lcs` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "lcs", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n lcs -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "lcs", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "lcs", "YOUR_CONSOLE_SCRIPT"]
